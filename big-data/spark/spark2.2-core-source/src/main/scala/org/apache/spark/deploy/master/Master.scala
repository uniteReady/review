...
private[deploy] class Master(
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case RegisterApplication(description, driver) =>
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        // master向Client端发送消息，告诉它任务信息注册成功
  ...
  private def startExecutorsOnWorkers(): Unit = {
    ...
    // 从waitingApps中取出来一个app目的是启动executor
    for (app <- waitingApps if app.coresLeft > 0) {
      val coresPerExecutor: Option[Int] = app.desc.coresPerExecutor

      // Filter out workers that don't have enough resources to launch an executor
      // 过滤掉那些没有足够资源来启动executor的worker,将空闲的worker放到前面先进行分配
      val usableWorkers = workers.toArray.filter(_.state == WorkerState.ALIVE)
        .filter(worker => worker.memoryFree >= app.desc.memoryPerExecutorMB &&
          worker.coresFree >= coresPerExecutor.getOrElse(1))
        .sortBy(_.coresFree).reverse

      //spreadOutApps 默认将应用程序分散到所有节点执行而不是在一台节点上
      //返回一个数组，其中包含分配给每个worker的内核数。
      //启动执行者有两种模式。第一次尝试将应用程序的执行器分散到尽可能多的worker上，而第二次尝试则相反(即在尽可能少的worker上启动它们)。
      //对于数据本地化的目的，前者通常更好，并且是默认值。
      val assignedCores = scheduleExecutorsOnWorkers(app, usableWorkers, spreadOutApps)

      // Now that we've decided how many cores to allocate on each worker, let's allocate them
      //决定了在每个worker上分配多少个内核后，开始进行分配启动Executor
  ...
  /**
   * Allocate a worker's resources to one or more executors.  将worker的资源分配给一个或多个executor
   * @param app the info of the application which the executors belong to。 executor所属的应用程序的信息
   * @param assignedCores number of cores on this worker for this application 这个应用程序在worker上分配的内核数
   * @param coresPerExecutor number of cores per executor 每个executor的核数
  ...
  private def schedule(): Unit = {
    ...
    // Drivers take strict precedence over executors Driver严格优先与executors
    ...
    //在work上启动executors执行任务
    startExecutorsOnWorkers()
  }

  private def launchExecutor(worker: WorkerInfo, exec: ExecutorDesc): Unit = {
    logInfo("Launching executor " + exec.fullId + " on worker " + worker.id)
    // 将Executor的信息加入到workerInfo中
    worker.addExecutor(exec)
    // Master向worker发送消息，告诉Worker启动Executor
    worker.endpoint.send(LaunchExecutor(masterUrl,
      exec.application.id, exec.id, exec.application.desc, exec.cores, exec.memory))
    // Master向Driver端的ClientEndpoint发送消息Executor已经被启动了
